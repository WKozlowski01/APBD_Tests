...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnAllSalesmen()
    {
        ...
        List<Emp> result = emps.Where(j=>j.Job.Equals("SALESMAN")).ToList(); 
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = (from e in emps
            where (from d in depts where d.Loc == "CHICAGO" select d.DeptNo).Contains(e.DeptNo) select e).ToList();
    ...
    public void ShouldSelectNamesAndSalaries()
    {
        ...
        var result = (from s in emps
                        select new{s.EName,s.Sal} ).ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result =from e in emps
            where e.Sal > (from inner in emps
                where inner.DeptNo == e.DeptNo
                select inner.Sal).Average()
            select e.EName;
